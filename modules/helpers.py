...
def split_bayer(bayer_array, bayer_pattern):
    ...
    :return: 4-element list of R, Gr, Gb, and B channel sub-arrays, (or R, G, B, IR) each is an np.ndarray(H/2, W/2)
    """
    ...
    # Different gap 
    gap = int(math.sqrt(len(rggb_indices)))
        
    for idx in rggb_indices:
        x0, y0 = idx
        sub_arrays.append(
            bayer_array[y0::gap, x0::gap]
        ) 
    return sub_arrays
...
def reconstruct_bayer(sub_arrays, bayer_pattern):
    ...
    bayer_array = np.empty(shape=(gap * height, gap * width), dtype=sub_arrays[0].dtype)

    for idx, sub_array in zip(rggb_indices, sub_arrays):
        x0, y0 = idx
        bayer_array[y0::gap, x0::gap] = sub_array
